...
# Todd Hunter (NRAO)
# Eric Villard (JAO)
# $Id: almaqa2csg.py,v 2.5 2023/02/14 12:24:46 dpetry Exp $
#
"""
...
def version(short=False):
    ...
    """
    myversion = "$Id: almaqa2csg.py,v 2.5 2023/02/14 12:24:46 dpetry Exp $"
    if (short):
...
CASA_VERSION_WITH_MS_BUGFIX = '6.2.0.97' # CAS-13373
CASA_VERSION_WITHOUT_SUBSX = '6.3.0'


def genQA2ReportPostLog(ms1, ms2, phase_cal, target, target_source, tsys_field, dospw, 
    ...
    s = 'Starting almaqa2repgen.generateQA2Report('
    if (ms1 != ''):
        s += "ms1='%s'," % (ms1)
    if (ms2 != ''):
        s += "ms2='%s'," % (ms2)
...
def generateQA2Report(ms1='', ms2='', phase_cal='', target='',
                      ...
                      showgui=False, montage='montage'):
    """
    The ALMA QA2 Report Generator
    (to be run on a per-execution-block basis after the calibration
    with the help of almaqa2csg.py is completed)

    Initial version:  Ed Fomalont, Feb 27, 2012
    streamlined, Amy Kimball, May 1, 2012
    updated instructions and other improvements, Feb 2014-2023  (Todd Hunter)
    ...
    target_source: the source ID of the science target as a string (e.g. '3')
         It is used by sfsdr.sensitivity_calculator and to auto-pick the tsys_field.
         The default is the first source with OBSERVE_TARGET intent.
    target: the field ID of the science target as a string (e.g. '3')
            It is used by sfsdr.target_spectrum, and for cleaning and plotuv.  
    ...
    ampcalspw: which spws to plot in ant_amp_temporal (default = all)
       ...
       set this to the flux calibrator's spws for bandwidth-switching
       datasets, e.g. '5,7,9,11'
       (But it attempts to find these automatically using msmd.)
    ...
    showgui: passed to plotms and plotcal calls
    useLocalAlmaHelper: if False, then use the version in casarecipes

    INSTRUCTIONS

    1. Run almaqa2csg.generateReducScript in an appropriate directory.
       Then perform the calibration with it by running the generated script
       step-wise and editing it where needed.
       When the calibration of the EB is complete, you can proceed to
       the next step and generate the QA2 report for the EB.

    2. Run generateQA2Report('msname')
    where msname is the name of the (unsplit) ms
    This creates the qa2_output directory, and runs all the qa2 scripts below.

    3. For bandwidth-switching datasets, you need to set phase_cal, target,
       dospw, ampcalspw, and phasecalspw.  Example:
       generateQA2Report(ms1='uid___A002_Xed4607_X16d5.ms', refAnt='DV03', 
                         phase_cal='2', target='4', 
                         dospw='25,27,29,31,33,35,37,39,41,43,45',
                         ampcalspw='5,7,9,11',phasecalspw='5,7,9,11')

    4. For Band-to-Band phase transfer (B2B) datasets, you need to set parameters
       according to the following pattern:
       generateQA2Report('uid___X.ms', ant_amp_temporal_caltable='uid___X.ms.split.ampli_inf',
                         ampcalspw=<HF SPWs, e.g. '43,45,47,49'>, 
                         ant_phase_temporal_caltable='uid___X.ms.split.phasehigh_int', 
                         phasecalspw=<same as ampcalspw>, phase_cal=<DGC field ID, e.g. '1'>,
                         checksourceAnalysis=False)
       NOTE w.r.t. B2B data: generateQA2Report does not know about B2B mode! 
            As a temporary measure the DGC is substituted as the 'phasecal' for the report.
            This means the quick image of the target will not be correctly phase calibrated. 
            So do not use this for any QA assessment
    ...
    print('*** Generating QA2 report ... ***')
    casalog.post('using '+version(), 'INFO', 'generateQA2Report')
    casalog.post('and '+aU.version(), 'INFO', 'generateQA2Report')

    if not showgui:
        pb.ioff()

    ms1 = ms1.rstrip('/')
    if ant_gain_check_caltable is None and bandpass_caltable != '':
        # so that ant_gain_check.txt will be produced for the QA2 report
        ant_gain_check_caltable = bandpass_caltable.replace('.split.bandpass','.split.ap_pre_bandpass')
        print("Setting ant_gain_check_caltable to: %s " % (ant_gain_check_caltable))

    genQA2ReportPostLog(ms1, ms2, phase_cal, target, target_source, tsys_field, dospw, 
                        refAnt, qa2_output_dir, uvrange, autoscale_yaxis, fdmSpwsToImage,
                        ant_gain_check_caltable, ant_amp_temporal_caltable,
                        ant_phase_temporal_caltable, wvrspw,avgchannel,
                        flaggedFraction, phaseDiff,useLocalAlmaHelper, ampcalspw, 
    ...
    phaseCalInfo = sfsdr.getPhaseCal(ms1)
    intentsAndSourcesInfo = sfsdr.getIntentsAndSourceNames(ms1)
    if (target_source == ''):
        ...
        mymsmd.open(ms1)
        if (aU.getCasaVersion() < casaVersionWithMSMDFieldnames):
    ...
    if (aU.getCasaVersion() >= casaVersionWithMSMD):
        mymsmd = aU.createCasaTool(msmdtool)
        mymsmd.open(ms2)
    if (target == ''):
    ...
    if (dospw == ''):
        if (aU.getCasaVersion() >= casaVersionWithMSMD):
            ...
            try:
                ...
                if (aU.getCasaVersion() >= casaVersionWithUndefinedFrame):
                    print("You are running casa %s but the data were calibrated in version < %s." % (aU.getCasaVersion(),casaVersionWithUndefinedFrame))
            ...
            spwInfo = list(sfsdr.getSpwInfo(ms1, caching=True).keys())
    ...
    if refAnt == '': refAnt = sfsdr.getRefAntenna(ms1)
    ...
    pols = sfsdr.findNumberOfPolarizations(ms2)
    #-------------

    print('# Get Colorful Diagram of Observations')
    sfsdr.listobs3(msName=ms1,figfile=qa2_output_dir+'obs_display.png')
    elevSummary1 = qa2_output_dir+'/'+ms1+'elevationSummary_vs_time.png'
    elevSummary2 = qa2_output_dir+'/'+ms1+'elevationSummary_vs_azim.png'
    elevSummary1 = aU.plotElevationSummary(ms1, plotfile=elevSummary1, overwrite=True, showgui=showgui)
    elevSummary2 = aU.plotElevationSummary(ms1, plotfile=elevSummary2, vs_azim=True, overwrite=True, showgui=showgui)

    print('# Get listings of experiment and antenna-config plot')
    print("# If mosaic, make this plot")
    sfsdr.mosaic_plot(ms2=ms2, qa2_output_dir=qa2_output_dir, showgui=showgui)

    if not aU.sevenMeterAntennasMajority(ms2):
        print('# Determine wvr correction from bandpass obs')
        print(" Running sfsdr.wvr_stat(ms1='%s', refAnt='%s', qa2_output_dir='%s', autoscale_yaxis=%s, spw='%s', scanintent='%s')" % (ms1,refAnt,qa2_output_dir,autoscale_yaxis, str(wvrspw), wvrScanIntent))
        sfsdr.wvr_stat(ms1=ms1, refAnt=refAnt, qa2_output_dir=qa2_output_dir, autoscale_yaxis=autoscale_yaxis, spw=wvrspw, scanintent=wvrScanIntent)

        # Produce a fixed-scale plot for display in analyzemscal
        print(" Running sfsdr.wvr_stat(ms1='%s', refAnt='%s', qa2_output_dir='%s', autoscale_yaxis=False, plotfile='wvr_plot_fixedscale.png', unwrap_phase=False, spw='%s', scanintent='%s')" % (ms1,refAnt,qa2_output_dir,str(wvrspw),wvrScanIntent))
        sfsdr.wvr_stat(ms1=ms1, refAnt=refAnt, qa2_output_dir=qa2_output_dir, autoscale_yaxis=False,
                      plotfile='wvr_plot_fixedscale.png', unwrap_phase=False, spw=wvrspw, scanintent=wvrScanIntent)

    print('# Tsys statistics and plots')
    print(version(True) + " Calling sfsdr.tsys_stat(ms1='%s', tsys_field='%s', makeplot=True, qa2_output_dir='%s')" % (ms1,tsys_field,qa2_output_dir))
    sfsdr.tsys_stat(ms1=ms1, tsys_field=tsys_field, makeplot=True, qa2_output_dir=qa2_output_dir, showgui=showgui)

    print('# listing of split data set')
    print(" Running sfsdr.listobs2(ms2='%s', makeplot=True, qa2_output_dir='%s', plotAntennasActualSize=True)" % (ms2, qa2_output_dir))
    sfsdr.listobs2(ms2=ms2, makeplot=True, qa2_output_dir=qa2_output_dir, plotAntennasActualSize=True)

    print('# Any antennas shadowed?')
    print(" Calling sfsdr.shadowed_ant(ms2='%s', qa2_output_dir='%s')" % (ms2,qa2_output_dir))
    sfsdr.shadowed_ant(ms2=ms2, qa2_output_dir=qa2_output_dir)

    print('# Check antenna amp of bandpass scan')
    print(" Running sfsdr.ant_gain_check(ms2='%s', qa2_output_dir='%s')" % (ms2, qa2_output_dir))
    sfsdr.ant_gain_check(ms2=ms2, qa2_output_dir=qa2_output_dir, caltable=ant_gain_check_caltable)

    print('# Check channel to channel rms of bandpass scan and plot outliers')
    print(" Running sfsdr.bandpass_rms(ms2='%s', refAnt='%s', qa2_output_dir='%s', caltable='%s')" % (ms2,refAnt,qa2_output_dir,bandpass_caltable))
    sfsdr.bandpass_rms(ms2=ms2, refAnt=refAnt, qa2_output_dir=qa2_output_dir, caltable=bandpass_caltable)

    print('# Make an spw average bandpass plots for atmosphere effects')
    print(" Running sfsdr.bandpass_plot(ms2='%s', qa2_output_dir='%s', caltable='%s')" % (ms2, qa2_output_dir, bandpass_caltable))
    sfsdr.bandpass_plot(ms2=ms2, qa2_output_dir=qa2_output_dir, caltable=bandpass_caltable)

    print('# Printout of flux density determinations')
    print(" Running sfsdr.flux_values(ms2='%s', qa2_output_dir='%s')" % (ms2, qa2_output_dir))
    sfsdr.flux_values(ms2=ms2, qa2_output_dir=qa2_output_dir)

    print('# Save flux densities for AQUA')
    print(" Running sfsdr.flux_values4AQUA(ms2='%s', qa2_output_dir='%s')" % (ms2, qa2_output_dir))
    sfsdr.flux_values4AQUA(ms2=ms2, qa2_output_dir=qa2_output_dir)

    print('# Plot temporal antenna amp calibration of phase cal')
    if (ant_amp_temporal_caltable == None):
        caltable = ms2+'.flux_inf'
        if phaseDiff == '':
            spwInfo = sfsdr.getSpwInfo(ms1, caching=True)
            spwIds = sorted(spwInfo.keys())
            spwInfo2 = sfsdr.getSpwInfo(ms1, intent = 'CALIBRATE_PHASE', caching=True)
    ...
    print(" Running sfsdr.ant_amp_temporal(ms2='%s', phase_cal='%s', caltable='%s', qa2_output_dir='%s', spw='%s')" % (ms2, phase_cal, caltable, qa2_output_dir, str(ampcalspw)))
    result = sfsdr.ant_amp_temporal(ms2=ms2, phase_cal=phase_cal, caltable=caltable, 
                                   qa2_output_dir=qa2_output_dir, spw=ampcalspw, nSpwsFlagged=nSpwsFlagged, phaseDiff=phaseDiff, showgui=showgui)
    if result != 0:
        print("Stopping.")
        return
    ...
    print(" Running sfsdr.ant_phase_temporal(ms2='%s', phase_cal='%s', caltable='%s', qa2_output_dir='%s', spw=%s)" % (ms2, phase_cal, caltable, qa2_output_dir, str(phasecalspw)))

    sfsdr.ant_phase_temporal(ms2=ms2, caltable=caltable, phase_cal=phase_cal, qa2_output_dir=qa2_output_dir, spw=phasecalspw)

    print('# Plot fluxcal object model and calibrated data')
    print("********\nRunning sfsdr.ampcal_uvdist(ms2='%s', qa2_output_dir='%s', spw='%s')" % (ms2,qa2_output_dir,ampcalspw))
    sfsdr.ampcal_uvdist(ms2=ms2, qa2_output_dir=qa2_output_dir, spw=ampcalspw, showgui=showgui, montage=montage) # PRTSPR-31340

    print('# Check uvdist and freq response of uv calibration phase cal')
    print(" Running sfsdr.phase_cal_check(ms2='%s', phase_cal='%s', qa2_output_dir='%s')" % (ms2,phase_cal,qa2_output_dir))
    sfsdr.phase_cal_check(ms2=ms2, phase_cal=phase_cal, qa2_output_dir=qa2_output_dir, showgui=showgui)

    print('# Obtain flagging statistics')
    print(" Running sfsdr.flag_stats(ms2='%s', qa2_output_dir='%s')" % (ms2, qa2_output_dir))
    flagStats = sfsdr.flag_stats(ms2=ms2, qa2_output_dir=qa2_output_dir, spw=dospw)
    ...
    print(" Running sfsdr.target_check(ms1='%s', ms2='%s', target='%s', target_source='%s', tsys_caltable='%s.tsys', tsys_field='%s', qa2_output_dir='%s', fdmSpwsToImage='%s', flaggedFraction=%f)" % (ms1, ms2, target, target_source, ms1, tsys_field, qa2_output_dir, fdmSpwsToImage, flaggedFraction))
    tcresult = sfsdr.target_check(ms1=ms1, ms2=ms2, target=target, 
                                 target_source=target_source,
                                 tsys_caltable=ms1+'.tsys', tsys_field=tsys_field,
                                 qa2_output_dir=qa2_output_dir, fdmSpwsToImage=fdmSpwsToImage,
                                 flaggedFraction=flaggedFraction, flagStats=flagStats, useLocalAlmaHelper=useLocalAlmaHelper, showgui=showgui)
    if (tcresult == None):
        print("Aborting")
        return
    cleanfield, cleanfieldname = tcresult
    print('#  Get a spectrum on the target')
    print(" Running sfsdr.target_spectrum(ms2='%s',target='%s',dospw='%s',qa2_output_dir='%s',uvrange='%s')" %  (ms2,target,dospw,qa2_output_dir,uvrange))
    sfsdr.target_spectrum(ms2=ms2, target=target, dospw=dospw, qa2_output_dir=qa2_output_dir,
                         uvrange=uvrange, avgchannel=avgchannel, showgui=showgui, montage=montage)

    if ((intentsAndSourcesInfo['OBSERVE_CHECK']['id'][0] != '')
        and (aU.getCasaVersion() >= '4.5')
    ...
    print(" Running sfsdr.glue_qa2(qa2_output_dir='%s', target='%s', pols='%s', cleanfield='%s', elevationSummary1='%s', elevationSummary2='%s', convert='%s')" % (qa2_output_dir, target, pols, cleanfield, os.path.basename(elevSummary1), os.path.basename(elevSummary2), convert))
    sfsdr.glue_qa2(qa2_output_dir, target, pols, cleanfield, cleanfieldname,
